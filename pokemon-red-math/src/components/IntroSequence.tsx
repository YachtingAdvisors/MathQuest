...
// ---------------------------------------------------------------------------
// Phase flow — mirrors Pokemon Red/Blue intro
// ---------------------------------------------------------------------------
...
enum Phase {
  ...
  RIVAL_NAME_ENTRY,
  RIVAL_NAME_CONFIRM,
  // Grade select (MathQuest addition)
  GRADE_INTRO,
  GRADE_SELECT,
  GRADE_CONFIRM,
  // Send off
...
const NameDisplay = styled.div`
  ...
  min-height: 6vh; padding: 0.5vh 1vh;
  @media (max-width: 1000px) {
...
const LetterBtn = styled.button<{ $active: boolean }>`
  ...
  cursor: pointer;
  @media (max-width: 1000px) {
    font-size: 8px; width: 14px; height: 14px; border-width: 1px;
  }
`;
...
const PresetBtn = styled.button<{ $active: boolean }>`
  ...
  padding: 0.5vh 2vh; cursor: pointer;
  @media (max-width: 1000px) {
    font-size: 7px; padding: 2px 6px; border-width: 1px;
  }
`;

const HintText = styled.div`
  font-family: "PokemonGB"; font-size: 1.8vh; color: #888;
  margin-top: 0.3vh;
...
const GradeGrid = styled.div`
  display: flex; flex-direction: column; width: 90%; max-width: 600px;
  overflow-y: auto;
`;
...
const IntroSequence = () => {
  ...
  // Determine which name we're editing
  ...
  const getText = (): string => {
    switch (phase) {
      ...
      case Phase.OAK_WORLD:
        return "My name is OAK! People call me the POKeMON PROF!";
      case Phase.OAK_POKEMON_INTRO:
        return "This world is inhabited by creatures called POKeMON!";
      case Phase.OAK_POKEMON_EXPLAIN:
        return "For some people, POKeMON are pets. Others use them for fights. Myself... I study POKeMON as a profession.";
      case Phase.OAK_ABOUT_YOU:
        return "First, what is your name?";
      case Phase.PLAYER_NAME_PROMPT:
        return "What is your name?";
      case Phase.PLAYER_NAME_CONFIRM:
        return `Right! So your name is ${playerName}!`;
      case Phase.RIVAL_INTRO:
        return `This is my grandson. He's been your rival since you were a baby. ...Erm, what is his name again?`;
      case Phase.RIVAL_NAME_PROMPT:
        return "What is his name?";
      case Phase.RIVAL_NAME_CONFIRM:
        return `That's right! I remember now! His name is ${rivalName}!`;
      case Phase.GRADE_INTRO:
        return `Now ${playerName}, one more thing. In this world, trainers sharpen their minds with math. What level of math are you ready for?`;
      case Phase.GRADE_CONFIRM: {
        const g = GRADE_CONFIGS[Number(grades[gradeIndex][0])];
        return `${g.label}! ${g.description}. A fine challenge!`;
      }
      case Phase.OAK_SENDOFF_1:
        return `${playerName}! Your very own POKeMON legend is about to unfold!`;
      case Phase.OAK_SENDOFF_2:
        return "A world of dreams and adventures with POKeMON awaits! Let's go!";
      case Phase.OAK_LAB_1:
        return `${playerName}! There are 3 POKeMON here. Haha! They are inside the POKe BALLS.`;
      case Phase.OAK_LAB_2:
        return "When I was young, I was a serious POKeMON trainer. In my old age, I have only 3 left. Choose one!";
      case Phase.STARTER_CONFIRM: {
        const s = STARTERS[starterIndex];
        return `So! You want ${s.name}, the ${s.type} POKeMON?`;
      }
      default:
        return "";
    }
  ...
  const advanceText = () => {
    const transitions: Partial<Record<Phase, Phase>> = {
      [Phase.OAK_WELCOME]: Phase.OAK_WORLD,
      [Phase.OAK_WORLD]: Phase.OAK_POKEMON_INTRO,
      [Phase.OAK_POKEMON_INTRO]: Phase.OAK_POKEMON_EXPLAIN,
      [Phase.OAK_POKEMON_EXPLAIN]: Phase.OAK_ABOUT_YOU,
      [Phase.OAK_ABOUT_YOU]: Phase.PLAYER_NAME_ENTRY,
      [Phase.PLAYER_NAME_CONFIRM]: Phase.RIVAL_INTRO,
      [Phase.RIVAL_INTRO]: Phase.RIVAL_NAME_ENTRY,
      [Phase.RIVAL_NAME_CONFIRM]: Phase.GRADE_INTRO,
      [Phase.GRADE_INTRO]: Phase.GRADE_SELECT,
      [Phase.GRADE_CONFIRM]: Phase.OAK_SENDOFF_1,
      [Phase.OAK_SENDOFF_1]: Phase.OAK_SENDOFF_2,
      [Phase.OAK_SENDOFF_2]: Phase.OAK_LAB_1,
      [Phase.OAK_LAB_1]: Phase.OAK_LAB_2,
      [Phase.OAK_LAB_2]: Phase.STARTER_SELECT,
    };
    const next = transitions[phase];
    if (next !== undefined) {
      setPhase(next);
      // Reset letter entry state when entering name phases
  ...
  const handleNameA = () => {
    if (onPresets) {
      ...
      setPhase(isNamingRival ? Phase.RIVAL_NAME_CONFIRM : Phase.PLAYER_NAME_CONFIRM);
      return;
    }
  ...
  const handleNameB = () => {
    if (currentName.length > 0) {
      setCurrentName(currentName.slice(0, -1));
    }
  };
  ...
  const confirmStarter = () => {
    ...
    dispatch(setRivalNameAction(rivalName));
  ...
  useEvent(Event.A, () => {
    ...
    if (phase === Phase.GRADE_SELECT) {
      setPhase(Phase.GRADE_CONFIRM);
      return;
    }
    if (phase === Phase.STARTER_SELECT) {
      setPhase(Phase.STARTER_CONFIRM);
      return;
    }
    if (phase === Phase.STARTER_CONFIRM) {
      confirmStarter();
      return;
    }
    advanceText();
  ...
  useEvent(Event.B, () => {
    ...
    if (phase === Phase.STARTER_SELECT) {
      setPhase(Phase.OAK_LAB_2);
    }
    if (phase === Phase.STARTER_CONFIRM) {
      setPhase(Phase.STARTER_SELECT);
    }
    if (phase === Phase.GRADE_SELECT) {
      setPhase(Phase.GRADE_INTRO);
    }
    if (phase === Phase.GRADE_CONFIRM) {
      setPhase(Phase.GRADE_SELECT);
    }
  });
  ...
  useEvent(Event.Up, () => {
    ...
    if (phase === Phase.GRADE_SELECT) {
      setGradeIndex((p) => Math.max(0, p - 1));
    }
  });
  ...
  useEvent(Event.Down, () => {
    ...
    if (phase === Phase.GRADE_SELECT) {
      setGradeIndex((p) => Math.min(grades.length - 1, p + 1));
    }
  });
  ...
  useEvent(Event.Right, () => {
    ...
    if (phase === Phase.STARTER_SELECT) {
      setStarterIndex(Math.min(STARTERS.length - 1, starterIndex + 1));
    }
  });
  ...
  // =========================================================================
  // RENDER: Name entry (shared for player & rival)
  // =========================================================================
  if (phase === Phase.PLAYER_NAME_ENTRY || phase === Phase.RIVAL_NAME_ENTRY) {
      ...
      <Overlay>
        <TopArea>
          ...
          <NameEntryWrap>
            ...
            <NameDisplay>
              {currentName}
              {"_".repeat(Math.max(0, 7 - currentName.length))}
            </NameDisplay>
            <LetterGrid>
              {LETTERS.map((letter, i) => (
                <LetterBtn key={letter} $active={!onPresets && letterIndex === i}>
                  {letter}
                </LetterBtn>
              ))}
            </LetterGrid>
            <PresetRow>
              {currentPresets.map((name, i) => (
                <PresetBtn key={name} $active={onPresets && presetIndex === i}>
                  {name}
                </PresetBtn>
              ))}
            </PresetRow>
  ...
  if (phase === Phase.GRADE_SELECT) {
    return (
      <Overlay>
        <TopArea>
          <OakImage src={oakPortrait} />
          <NameLabel style={{ marginBottom: "1vh" }}>
          ...
          <GradeGrid>
            {grades.map(([key, config], index) => (
              <GradeRow key={key} $active={index === gradeIndex}>
                <GradeArrowSlot>
                  <Arrow menu show={index === gradeIndex} />
                </GradeArrowSlot>
  ...
  // =========================================================================
  // RENDER: Dialogue phases (Oak talking)
  // =========================================================================
  return (
    <Overlay>
      <TopArea>
